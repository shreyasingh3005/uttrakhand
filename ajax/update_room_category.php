<?php
/** ajax/update_room_category.php — Edit existing room category */
...
$rChk = $pdo->prepare('SELECT id,hotel_id FROM hotel_room_categories WHERE id=?');
...
try {
    ...
    $pdo->prepare("UPDATE hotel_room_categories SET name=?,bed_type=?,room_size=?,total_rooms=?,available_rooms=?,extra_bed_allowed=?,extra_bed_price=?,max_extra_beds=?,updated_at=NOW() WHERE id=?")
        ->execute([$name,$bed,$size,$total,$avail,$eb_on,$eb_pr,$eb_mx,$room_id]);
    ...
    foreach (MEAL_CODES as $code) {
        $price = isset($prices[$code]) ? (float)$prices[$code] : 0.0;
        $plan_id = get_plan_id($pdo, $code);
        if ($plan_id > 0 && $price > 0) $prStmt->execute([$hotel_id,$room_id,$plan_id,$price]);
    }
