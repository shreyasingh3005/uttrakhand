<?php
/** ajax/save_room.php — Create new room category for a hotel */
...
$name     = s($d['name']       ?? '');
if ($hotel_id <= 0) hl_err('Valid hotel_id is required.');
...
try {
    ...
    $rcStmt->execute([$hotel_id, $name, $bed, $size, $total, $avail, $eb_on, $eb_price, $eb_max]);
    ...
    if (!empty($prices)) {
        ...
        foreach ($prices as $code => $price) {
            ...
            if ($plan_id) {
                $mp->execute([$hotel_id, $room_id, $plan_id, (float)$price]);
            }
        }
    }
